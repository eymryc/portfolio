...
export async function generateMetadata({ params }: PageProps) {
  ...
  const baseTitle = seo?.title?.trim() || profile?.name?.trim() || "Portfolio";
  return {
    title: page === "index" ? baseTitle : `${baseTitle} — ${page}`,
    description: seo?.description?.trim() || profile?.bio?.trim() || undefined,
  };
}

export default async function PublicPortfolioSubPage({ params }: PageProps) {
  const { slug, page } = await params;
  const data = await getPortfolio(slug);
